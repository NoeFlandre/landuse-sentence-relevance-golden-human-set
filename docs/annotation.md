...
The V2 session is at `annotations-v2.jsonl`, candidate progress at `candidate-progress-v2.json`, and the final pool at `candidate-pool-v2.json` on the Seagate project drive by default. The failed pre-optimization checkpoint is preserved as `candidate-progress-v2-failed-20260829.json`. V1 files remain separate. Raw streamed rows are not written locally.
