...
The first V2 build scanned 227,521 website rows but found only 213 valid English candidate cells, so it stopped before the required 256 website candidates and failed with `need enough disjoint source cells`. One malformed remote Parquet shard was also skipped because its Arrow batch size was zero. The root cause was an early-stop rule that treated the per-cell row cap as a completed candidate-cell quota. Its checkpoint is preserved as `candidate-progress-v2-failed-20260829.json`. The recovery path now counts valid candidates, prioritizes paragraph-like rows, reuses up to 16 compact discovery rows per cell, and checkpoints candidates at `candidate-progress-v2.json` on the Seagate. A restart can reuse completed source candidates without repeating their model work; raw streamed rows are still never persisted.

A second retry reached Wikipedia section splitting but produced no checkpoint after 19 minutes; the first batch was still inside SaT inference. Its zero-candidate checkpoint is preserved as `candidate-progress-v2-failed-20260829-section-bound.json`. The current path bounds Wikipedia sections to 800 characters before inference and uses bounded SaT batches. Remote catalog requests also force IPv4, retry once, and fail after 20 seconds instead of hanging.
