...
The V2 app streams two public Hugging Face datasets, keeps English sentences from inside source text blocks when available, and presents unique-cell candidates selected across the world with deterministic H3 maximin spacing. One human annotator assigns Yes or No relevance labels. The V2 runtime benchmark is `data/benchmark/v2-adjudicated.csv`, the human V2 set with its three-rater disagreements adjudicated. The finalized V3 annotation benchmark is `data/benchmark/v3/final/v3-final.csv`; its review and resolution trail is documented separately. Every data file is listed in the [Data catalogue](results.md).

The V3 benchmark is also released as 85 language-aligned CSV files. See the
[V3 multilingual benchmark](v3-translations.md) for the translation method,
file layout, integrity contract, and geographic coverage map.
...
A follow-up 300-row set is designed, and its oversized candidate pool, geographic preflight, and deterministic annotation seed are shipped; annotation remains paused. Its sources, quotas, storage, and runbook are described in [V3 workflow](v3.md).
